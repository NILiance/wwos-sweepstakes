...
import { GenerateFieldButton } from "./generate-button";

export const metadata = { title: "Bracket Control — Admin" };
...
export default async function AdminBracketPage({
  ...
  if (sw.game_mode !== "bracket") {
    return (
      <div className="max-w-xl">
        <h2 className="text-lg font-bold">Bracket control</h2>
        <p className="mt-2 text-sm text-muted">
          This pool&apos;s game mode is{" "}
          <span className="font-semibold">{sw.game_mode}</span>, not a bracket
          challenge. Set its mode to &quot;bracket&quot; when creating it.
        </p>
      </div>
    );
  }
  ...
  let truthPicks: Record<number, string> = {};
  if (field) {
      ...
      {!field ? (
        ...
        <>
          <p className="mt-2 text-sm text-muted">
            ...
            clicking real winners as games finish — every entrant&apos;s score
            updates instantly.{" "}
